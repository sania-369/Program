...
"""
🌀 ETVP v12.9 TANH — Чистый TANH. Версия для сравнения.
Тот же код, что и v12.9 FFS (clip), но с чистым tanh в Z-принципе.

Цель: сравнить clip и tanh на одних и тех же данных.
"""
...
# =============================================================================
# 1. Z-ПРИНЦИП (ЧИСТЫЙ TANH)
# =============================================================================

def etve_tanh_limit(C, c_min=GLOBAL_C_MIN, c_max=GLOBAL_C_MAX):
    """
    Z-принцип: чистый tanh-демпфер.
    Сжимает C к центру диапазона.
    """
...
def demo_ffs_calibration(n_steps=100_000, log_every=10_000):
    print("=" * 80)
    print("🌀 ETVP v12.9 TANH — Чистый TANH")
    print(f"   Тактов: {n_steps:,}")
    print("=" * 80)

    model = ETVEComplexCoreV129Tanh(memory_depth=100)
    print(f"\n🔧 База (C = 1):")
    ...
    plt.tight_layout()
    plt.savefig('etvp_ffs_v129_tanh_100k.png', dpi=150)
    plt.show()
